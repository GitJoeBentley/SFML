...
Player::Player(const string& name_, Grid& grid_, Sounds& sounds_, sf::RenderWindow& win)
    : name(name_), location(sf::Vector2i(-1,0)), grid(grid_), sounds(sounds_), window(win), countdown(NumRows * NumCols / 40), maxRow(0), maxCol(-1)
...
{
    player.setPosition(UpperLeft.x + CellWidth * location.x, UpperLeft.y + CellWidth * location.y);
    window.draw(player);
}
...
Wall::Type Player::move(Direction direction)
{
    if ((location.y == 0 && direction == Up) ||
            ...
            (location.x == NumCols-1 && location.y != NumRows-1 && direction == Right))
    {
...
Wall::Type Player::processMove(const sf::Vector2i& newLocation)
{
    ...
    Wall* ptrCell = grid.getCell(newLocation.y, newLocation.x);
    if (!ptrCell)
    {
    ...
    else
    {
        ...
        if (cellType == Wall::Rubber)
        {
            ...
            bounce();
            addToPath(100 * newLocation.y + newLocation.x);
            return cellType;
        }
        ...
        else    // Hit a wall
        {
            ...
            {
                delete grid.getCell(newLocation.y, newLocation.x);
                grid.getCell(newLocation.y, newLocation.x) = nullptr;
                location.x = newLocation.x;
                location.y = newLocation.y;
            }
        }
    ...
    updateScore();
    addToPath(100 * newLocation.y + newLocation.x);
    //cout << "New location:  row = " << newLocation.y << "  col = " << newLocation.x << endl;
...
void Player::bounce()
{
    ...
    location = newLoc;
    addToPath(100 * newLoc.y + newLoc.x);
}
...
void Player::draw_path(sf::RenderWindow& window) const
{
    ...
    for (unsigned i = 0; i < path.size(); i++)
    {
        y = path[i] / 100;
        x = path[i] % 100;
        ptrCell = grid.getCell(y,x);

        if (ptrCell) window.draw(*ptrCell);
        else
        {
            empty.setPosition(UpperLeft.x + x * CellWidth, UpperLeft.y + y * CellWidth);
            window.draw(empty);
        }
    }
}
