...
//      border(sf::Vector2f(GameWindowSize, GameWindowSize)),
      border(sf::Vector2f(NumCols * 20.0f, NumRows * 20.0f)),
      door1(CellSize),door2(CellSize),
      titleText(name_ + "'s Hidden Maze Game - Version 3.0", Fonts::Kristan, 36),
      statusText("Time 60\nBruises 0\nScore 0", Fonts::Courier, 24 ),
      displayMaze(false)
{
    border.setOutlineThickness(CellWidth);
    borderTexture.loadFromFile(BorderImageFile);
    sf::FloatRect rect = border.getLocalBounds();
    border.setOrigin(rect.left + rect.width / 2.0f, rect.top + rect.height / 2.0f);
    border.setTexture(&borderTexture);
    border.setOutlineColor(sf::Color(sf::Color(20,130,10)));
    border.setPosition(MidWindow);
    sf::FloatRect borderBounds = border.getGlobalBounds();
    UpperLeft = sf::Vector2f(borderBounds.left + CellWidth,borderBounds.top + CellWidth);

    door1.setFillColor(sf::Color(0,0,0));
    door1.setPosition(UpperLeft.x - CellWidth, UpperLeft.y);
    door2.setFillColor(sf::Color(sf::Color::Green));
    door2.setPosition(UpperLeft.x + (NumCols) * CellWidth, UpperLeft.y + (NumRows - 1) * CellWidth);

    arrowTexture.loadFromFile(ArrowImageFile);
    arrow1.setTexture(arrowTexture);
    arrow2.setTexture(arrowTexture);
    //arrow1.setPosition(borderBounds.left - CellWidth, borderBounds.top + CellWidth);
    arrow1.setPosition(UpperLeft.x - 2 * CellWidth, UpperLeft.y);
    arrow2.setPosition(UpperLeft.x + (NumCols + 1) * CellWidth, UpperLeft.y + (NumRows - 1) * CellWidth);

    titleText.setPosition(borderBounds.left, CellWidth);
    titleText.setFillColor(sf::Color(250,150,200));

    statusText.setPosition(borderBounds.left + .9f * borderBounds.width, CellWidth);
    ...
    timeBar.setOutlineThickness(3.0f);
    timeBar.setOutlineColor(sf::Color::Red);
    timeBar.setPosition(borderBounds.left, CellWidth * 4.f);
}
